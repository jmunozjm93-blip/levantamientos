...
- Solo cuentan las **tiendas que están en la hoja de supervisores** con un supervisor real: si la columna trae `Z`/`ZZZZZ` (venta a distancia, bodegas, administrativos, tiendas sin asignar), `-`, `N/A` o el nombre de alguien que ya no es supervisor (Ximena Soto), la tienda se descarta. Las columnas de sucursales que no aparecen ahí (bodegas, .com, tiendas nuevas sin asignar) se descartan y se listan en el resumen de carga como "sin supervisor".
